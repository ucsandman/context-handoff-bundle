...
def build_relations(
    projects: list[str], findings: list[str], evidence: list[str]
) -> list[dict]:
    relations: list[dict] = []
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    write_json(
        ...
        build_bundle_metadata(
            ...
            notes="Generated from structured markdown notes.",
        ),
    )
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    markdown_ok = markdown_path.exists() and markdown_path.read_text(
        encoding="utf-8"
    ).strip().startswith("# Context Handoff:")
    resume_ok = (
        resume_path.exists()
        and len(resume_path.read_text(encoding="utf-8").strip()) > 0
    )

    json_failures = [
        r for r in file_results if not r["json_valid"] or r["schema_valid"] is False
    ]
...
def cmd_save(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "update", False):
        ...
        for store_fn in [lambda: get_repo_local_store(Path.cwd()), get_global_store]:
            ...
            resolved = s.resolve(
                args.update
                if isinstance(args.update, str) and args.update is not True
                else None,
...
def cmd_delete(args: argparse.Namespace) -> int:
    ...
    for store_fn in [lambda: get_repo_local_store(cwd), get_global_store]:
        ...
        if resolved and not isinstance(resolved, list):
            ...
            print(
                json.dumps(
                    {"deleted": True, "id": resolved["id"], "path": str(bundle_path)}
                )
            )
            deleted = True
            break
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_init.add_argument(
        "--output", default="docs/context-handoffs", help="bundle output root"
    )
    p_init.add_argument("--slug", default="untitled-handoff", help="bundle slug")
    p_init.add_argument(
        "--with-graph", action="store_true", help="also create graph.json"
    )
    p_init.add_argument(
        "--force", action="store_true", help="overwrite existing files from templates"
    )
    p_init.set_defaults(func=cmd_init)

    # --- generate ---
    p_generate = sub.add_parser(
        "generate", help="generate a starter bundle from structured notes"
    )
    ...
    p_validate = sub.add_parser(
        "validate", help="validate required bundle files and JSON schema conformance"
    )
    ...
    p_save = sub.add_parser(
        "save", help="save a durable handoff bundle to the registry"
    )
    ...
    p_save.add_argument(
        ...
        dest="global_store",
        action="store_true",
        help="save to global store",
    )
    p_save.add_argument(
        "--repo-local",
        dest="repo_local",
        action="store_true",
        help="save to repo-local store",
    )
    p_save.add_argument(
        "--update",
        nargs="?",
    ...
    p_load = sub.add_parser(
        "load", help="load a saved handoff bundle and emit resume context"
    )
    p_load.add_argument(
        "query", nargs="?", default=None, help='bundle id, slug, or "latest"'
    )
    p_load.add_argument(
        "--deep", action="store_true", help="deep load mode with full context"
    )
    p_load.add_argument(
        "--json",
        dest="json_output",
        action="store_true",
        help="output JSON instead of resume text",
    )
    ...
    p_list.add_argument(
        "--repo-only", action="store_true", help="only show repo-local bundles"
    )
    p_list.add_argument(
        "--global-only", action="store_true", help="only show global bundles"
    )
    ...
    p_score = sub.add_parser(
        "score", help="recompute quality score for a bundle directory"
    )
    ...
    p_prune = sub.add_parser(
        "prune", help="remove old duplicate bundles, keeping newest per slug"
    )
    p_prune.add_argument(
        "--keep", type=int, default=1, help="how many to keep per slug (default: 1)"
    )
    p_prune.add_argument(
        "--repo-only", action="store_true", help="only prune repo-local store"
    )
    p_prune.add_argument(
        "--global-only", action="store_true", help="only prune global store"
    )
...
def main() -> int:
    # Windows consoles default to cp1252, which crashes on bundle content
    # containing Unicode (arrows, multiplication signs, etc.).
