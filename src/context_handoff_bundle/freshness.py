    ...
    if created_at:
        try:
            ...
            if age.total_seconds() > 7 * 24 * 3600:
                warnings.append(
                    f"Bundle is {result['age_hours']:.0f} hours old (>7 days)"
                )
                result["fresh"] = False
            elif age.total_seconds() > 24 * 3600:
                warnings.append(
                    f"Bundle is {result['age_hours']:.0f} hours old (>1 day)"
                )
        except Exception:
            pass
    ...
    if saved_branch:
        try:
            ...
            if not result["branch_match"]:
                warnings.append(
                    f'Branch changed: bundle was on "{saved_branch}", now on "{current_branch}"'
                )
        except Exception:
            pass
        ...
        try:
            ...
            if not result["commit_match"]:
                ...
                try:
                    ...
                    warnings.append(
                        f"Repo advanced {count} commit(s) since bundle was saved"
                    )
                except Exception:
                ...
                if isinstance(evidence, list):
                    for item in evidence:
                        ...
                        if path_str and not path_str.startswith("http"):
                            path_str = _normalize_evidence_path(path_str)
                            if path_str is None:
                                continue
                            p = Path(path_str)
                            if not p.exists():
