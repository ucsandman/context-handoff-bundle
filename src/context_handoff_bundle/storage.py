...
class BundleStore:
    ...
    def list_entries(
        ...
        if repo_root:
            ...
            entries = [
                e for e in entries if _normalize_path(e.get("repo_root", "")) == norm
            ]
        if tag:
    ...
    def resolve(
        ...
        if not query or query == "latest":
            if repo_root:
                ...
                repo_entries = [
                    e
                    for e in entries
                    if _normalize_path(e.get("repo_root", "")) == norm
                ]
                if repo_entries:
        ...
        partial = [
            e
            for e in entries
            if query.lower() in e.get("slug", "").lower()
...
def get_global_store() -> BundleStore:
    # CONTEXT_HANDOFF_HOME overrides the global store location
    # (lets tests and CI avoid touching the real user store).
    override = os.environ.get("CONTEXT_HANDOFF_HOME")
    if override:
        return BundleStore(Path(override).expanduser())
...
def save_bundle(
    ...
    entry: dict[str, Any] = {
        ...
        if DEFAULT_REPO_LOCAL_DIR in str(store.store_path)
        else "global",
    }
