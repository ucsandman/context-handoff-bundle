...
def compose_resume(
    ...
    # ── CONFIDENCE SUMMARY ──
    confidence_parts = _compute_section_confidence(
        summary, findings, at_risk_ids, drift
    )
    if confidence_parts:
        parts.append("")
    ...
    if drift and drift.get("has_drift"):
        ...
        if not freshness.get("fresh", True):
            parts.append(
                "- **This handoff may be stale. Verify claims before acting on them.**"
            )

    # ── PRESSURE: what matters most ──
    # Blocking questions only. Recommendations appear once, in Next Moves --
    # printing them here too tripled them in the output (Drift + Pressure +
    # Next Moves) and bloated every load.
    ...
    # ── MUST REVERIFY: what not to trust blindly ──
    # Save mirrors open questions into must_reverify, so filter exact dupes
    # here -- they get their own section below.
    must_reverify = resume_instr.get("must_reverify", [])
    question_texts = {q.get("question", "").strip().lower() for q in questions}
    must_reverify = [
        item
        for item in must_reverify
        if str(item).strip().lower() not in question_texts
    ]
    stale_evidence = []
    if freshness:
...
def _compute_section_confidence(
    ...
    # Evidence confidence -- 'verified' beats 'ok'; 'n/a' anchors (URLs,
    # commands, prose) are not file-checkable and don't count against it.
    if drift and drift.get("evidence_status"):
        checkable = [e for e in drift["evidence_status"] if e["status"] != "n/a"]
        good = sum(1 for e in checkable if e["status"] in ("ok", "verified"))
        verified = sum(1 for e in checkable if e["status"] == "verified")
        total = len(checkable)
        if total > 0:
            if good == total:
                label = "verified" if verified == total else "strong"
                sections.append(("Evidence", label))
            elif good > total / 2:
                sections.append(
                    ("Evidence", f"partial ({total - good}/{total} affected)")
                )
            else:
                sections.append(("Evidence", "WEAK"))

    # Recommendations confidence -- only degraded by real anchor impact,
    # never by raw commit volume.
    recs = summary.get("recommendations", [])
    if recs:
