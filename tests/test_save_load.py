...
def tmp_store(tmp_path, monkeypatch):
    """Redirect the global store to a temp dir so tests never pollute
    the real ~/.context-handoff-bundles store."""
    store_dir = tmp_path / "test-store"
    store_dir.mkdir()
    monkeypatch.setenv("CONTEXT_HANDOFF_HOME", str(store_dir))
    return store_dir


class TestSaveCommand:
    def test_save_minimal(self, tmp_store):
        """Save with just a title produces a valid bundle in the temp store."""
        ...
        assert data["token_estimates"]["bundle_tokens"] > 0
        assert data["token_estimates"]["heuristic"] == "chars/4"
        # Saved into the isolated store, not the real ~/.context-handoff-bundles
        assert Path(data["path"]).parent == tmp_store
        # Clean up
...
class TestLoadCommand:
    def test_load_latest(self):
        ...
        assert "[tokens] resume:" in result.stdout
    ...
    def test_load_by_id(self):
        ...
        save_result = run_cli(
            "save", "--title", "ID Test", "--slug", "id-test", "--global"
        )
        if save_result.returncode != 0:
            pytest.skip("Save failed")
        save_data = json.loads(save_result.stdout)
...
class TestEndToEnd:
    def test_save_list_show_load_cycle(self):
        ...
        # Save
        save_result = run_cli(
            "save", "--title", "E2E Test", "--slug", "e2e-test", "--global"
        )
        ...
        try:
            ...
            assert (
                "e2e-test" in list_result.stdout.lower()
                or "E2E Test" in list_result.stdout
            )
